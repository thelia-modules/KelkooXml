...
namespace KelkooXml\EventListener;

use KelkooXml\Controller\FieldAssociationController;
...
class KelkooXmlListener implements EventSubscriberInterface
{
    ...
    /**
     * KelkooXmlListener constructor.
     * @param EventDispatcher $dispatcher
     * @param ContainerInterface $container
     */
    public function __construct(EventDispatcher $dispatcher, ContainerInterface $container, URL $urlManager)
    {
    ...
    /**
     * @param KelkooGenerateXmlEvent $event
     * @return string
     * @throws \Exception
     */
    public function generateXml(KelkooGenerateXmlEvent $event)
    {
        $feed = $event->getFeed();
        ...
        try {
            ...
            return $event->setXmlContent($content);
        }
        catch (\Exception $ex) {
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    protected function renderXmlOnePse(KelkooxmlFeed $feed, &$pse, $shippingStr, $checkAvailability, $storeInfos)
    {
        $str = '<product>'.PHP_EOL;
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    protected function getProductItems(KelkooxmlFeed $feed, $limit = null, $offset = null)
    {
        $sql = 'SELECT 
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    protected function injectUrls(&$pseArray, KelkooxmlFeed $feed)
    {
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    protected function injectTaxedPrices(&$pseArray, KelkooxmlFeed $feed)
    {
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    protected function injectCustomAssociationFields(&$pseArray, KelkooxmlFeed $feed)
    {
        $attributesArray = [];
        $featuresArray = [];
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    protected function injectAttributesInTitle(&$pseArray, KelkooxmlFeed $feed)
    {
    ...
    protected function injectImages(&$pseArray)
    {
        foreach ($pseArray as &$pse) {
            if ($pse['IMAGE_NAME'] != null) {
                ...
                $this->dispatcher->dispatch(TheliaEvents::IMAGE_PROCESS, $imageEvent);
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    protected function buildShippingArray(KelkooxmlFeed $feed)
    {
        $resultArray = [];

        $shippingInfoArray = $this->getShippings($feed);

        foreach ($shippingInfoArray as $moduleTitle) {
            $shippingItem = [];
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    protected function getShippings(KelkooxmlFeed $feed)
    {
        ...
        foreach ($search as $deliveryModule) {
            ...
            $deliveries[] = $deliveryModule->getTitle();
    ...
    {
        return[
            KelkooGenerateXmlEvent::GENERATE_XML_EVENT => 'generateXml'
        ];
    }
